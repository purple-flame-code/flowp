...
type RiesgoClave = "Peligro de fuga" | "Obstaculización de la investigación" | "Peligro para la víctima" | "Peligro para la comunidad";
...
// ===========================================
//  Catálogo: indicadores por riesgo (basados en CPP PAN – arts. 221, 227 y concordantes)
//  Nota: referencia genérica para evitar citar números erróneos en vista previa. Ajustable si el usuario quiere literalidad de artículos.
// ===========================================
const INDICADORES: Record<RiesgoClave, Indicador[]> = {
  "Peligro de fuga": [
    { id: "sin-domicilio", nombre: "Falta de domicilio/residencia estable", descripcion: "Ausencia de domicilio fijo o información no verificable" },
    { id: "escasos-lazos", nombre: "Escaso arraigo familiar/laboral", descripcion: "Sin empleo formal ni núcleo familiar primario asentado" },
    { id: "pena-elevada", nombre: "Pena esperada elevada", descripcion: "Incentivo objetivo a sustraerse del proceso" },
    { id: "recursos-fuga", nombre: "Facilidades materiales para ocultarse/salir del país", descripcion: "Recursos económicos/medios logísticos" },
    { id: "hist-evasion", nombre: "Antecedentes de incomparecencia/evasión", descripcion: "Quebrantamientos, rebeldías o cambios de domicilio no informados" },
  ],
  "Obstaculización de la investigación": [
    { id: "amenazas-victima", nombre: "Intimidación a víctima/testigos", descripcion: "Conductas recientes orientadas a coaccionar" },
    { id: "destruccion-prueba", nombre: "Riesgo de destrucción de evidencia", descripcion: "Capacidad y oportunidad para alterar o suprimir prueba" },
    { id: "coordinacion-coimputados", nombre: "Coordinación con coimputados", descripcion: "Estructura para acordar versiones o eludir diligencias" },
    { id: "acceso-expediente", nombre: "Acceso privilegiado a fuentes de prueba", descripcion: "Posición o cargo que facilite influir indebidamente" },
  ],
  "Peligro para la víctima": [
    { id: "proximidad", nombre: "Proximidad geográfica/personal", descripcion: "Cercanía que incremente la exposición de la víctima" },
    { id: "violencia-previa", nombre: "Antecedentes de violencia o amenazas", descripcion: "Historial reciente con la víctima o su familia" },
    { id: "quebrantamiento", nombre: "Quebrantamiento de medidas previas", descripcion: "Incumplimientos de órdenes de alejamiento u otras" },
  ],
  "Peligro para la comunidad": [
    { id: "organizacion", nombre: "Vinculación a organización criminal", descripcion: "Recursos y logística para reiterar" },
    { id: "pluralidad-delitos", nombre: "Pluralidad/naturaleza de delitos", descripcion: "Hechos múltiples o especialmente graves" },
    { id: "condenas-vigentes", nombre: "Condenas vigentes relevantes", descripcion: "Ejecución pendiente u otras" },
  ],
};
...
function puntuarArraigo(a: Arraigo) {
  ...
  if (a.tiempoResidenciaMeses >= 24) score += 15; else if (a.tiempoResidenciaMeses >= 6) score += 8;
  ...
  if (a.pasaporteVigente) score -= 5; // riesgo potencial si hay otros factores
...
function redactarConclusiones(rol: Rol, filas: FilaRiesgo[], arraigoScore: number) {
  ...
  if (rol === "Fiscalía") {
    const base = `Conforme al CPP (arts. 221, 227 y concordantes), los riesgos identificados justifican una medida de cautela idónea, necesaria y proporcional.`;
    if (graves.length > 0 || (moderados.length >= 2 && nivelArraigo !== "Alto")) {
      return base + ` Se solicita la imposición de detención provisional o, subsidiariamente, ${MEDIDAS_ALTERNATIVAS.slice(0,3).join(", ")}, dada la concurrencia de: ` + graves.map(g=>g.riesgo).join(", ") + (moderados.length? ` y riesgos moderados concurrentes`:``) + `, y un arraigo ${nivelArraigo.toLowerCase()}.`;
    }
    return base + ` Se proponen medidas menos gravosas: ${MEDIDAS_ALTERNATIVAS.slice(0,4).join(", ")}, en atención a un arraigo ${nivelArraigo.toLowerCase()} y la ausencia de riesgos graves.`;
  }
  if (rol === "Defensa") {
    const base = `Bajo los principios de proporcionalidad y subsidiariedad (CPP art. 221), y atendiendo al control judicial estricto de afectación de derechos fundamentales, se plantea:`;
    if (graves.length === 0 && nivelArraigo !== "Bajo") {
      return base + ` sustitución por medidas no privativas: ${["Presentación periódica", "Prohibición de acercamiento", "Prohibición de salida del país"].join(", ")}, pues el arraigo es ${nivelArraigo.toLowerCase()} y los indicadores no alcanzan umbral de gravedad.`;
    }
    return base + ` en caso de estimarse algún riesgo, su neutralización con medidas específicas y verificables (${MEDIDAS_ALTERNATIVAS.slice(0,5).join(", ")}), descartando la detención por insuficiencia de elementos y por existir alternativas idóneas.`;
  }
  // Juez
...
// ===========================================
function Box({ children, className = "" }: any) { return <div className={`bg-slate-900/50 border border-white/10 rounded-2xl ${className}`}>{children}</div>; }
...
function Section({ children, className = "" }: any) { return <div className={`p-4 ${className}`}>{children}</div>; }
...
export default function RiesgosCautelares() {
  ...
  const baseFilas: FilaRiesgo[] = useMemo(()=> (Object.keys(INDICADORES) as RiesgoClave[]).map((rk)=>({
    riesgo: rk,
  ...
  function toggleIndicador(idx: number, idInd: string) {
    setFilas(prev => prev.map((f,i)=> i!==idx? f : ({...f, indicadores: {...f.indicadores, [idInd]: !f.indicadores[idInd]}})));
  }
  function setEvidencia(idx:number, v:string){ setFilas(prev=> prev.map((f,i)=> i!==idx? f : ({...f, evidencia:v}))); }
  function setValoracion(idx:number, v:Valoracion){ setFilas(prev=> prev.map((f,i)=> i!==idx? f : ({...f, valoracion:v}))); }
  function setMedidas(idx:number, arr:string[]){ setFilas(prev=> prev.map((f,i)=> i!==idx? f : ({...f, medidasSugeridas:arr}))); }
  function setPropuesta(idx:number, v:string){ setFilas(prev=> prev.map((f,i)=> i!==idx? f : ({...f, propuesta:v}))); }
  ...
  function generarInforme(rol: Rol, meta: any, filas: FilaRiesgo[], a: Arraigo, aScore:number) {
    ...
    const arr = `Arraigo: ${clasificarArraigo(aScore)} (puntaje ${aScore}/100). Detalle: domicilio ${a.domicilioFijo?"fijo":"no verificado"}; residencia ${a.tiempoResidenciaMeses} meses; empleo ${a.empleoFormal?"formal":"no formal"}${a.contratoIndefinido?", contrato indefinido":""}; familia primaria ${a.familiaPrimariaEnPA?"en Panamá":"no asentada"}; estudios ${a.estudiosActivos?"activos":"no aplica"}; antecedentes de evasión ${a.antecedentesEvasion?"sí":"no"}; pasaporte ${a.pasaporteVigente?"vigente":"no consta"}; recursos para fugarse: ${a.recursosParaFugarse}.\n\n`;

    const cuerpo = filas.map(f=>{
      const inds = Object.entries(f.indicadores).filter(([_,v])=>v).map(([id])=> INDICADORES[f.riesgo].find(x=>x.id===id)?.nombre).filter(Boolean).join(", ") || "(sin indicadores marcados)";
  ...
  async function descargarPDF() {
    ...
    try {
      ...
      const blob = await mod.generatePDF({ title: `Matriz de riesgos – ${meta.numeroCausa}`, content: contentStyled, branding: {} });
      mod.downloadPDF(blob, `matriz-riesgos-${Date.now()}.pdf`);
      return;
    } catch(e) { console.warn("pdf-generator no disponible, usando DOC", e); }
    // Fallback DOC
    const safe = (s:string)=>s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
    const html = `<!doctype html><html><head><meta charset=\"utf-8\"><style>body{white-space:pre-wrap;font-family:Times New Roman}</style></head><body>${safe(informe)}</body></html>`;
    const blob = new Blob([html], { type: "application/msword" });
    const url = URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`matriz-riesgos-${Date.now()}.doc`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  }

  function descargarTXT() {
    if (!informe.trim()) return;
    const blob = new Blob([informe], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`matriz-riesgos-${Date.now()}.txt`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  }
  ...
  function test(id: string) {
    if (id === "fiscalia-riesgo-alto") {
      ...
      setArraigo({ domicilioFijo:false, tiempoResidenciaMeses:4, empleoFormal:false, contratoIndefinido:false, familiaPrimariaEnPA:false, estudiosActivos:false, antecedentesEvasion:true, pasaporteVigente:true, recursosParaFugarse:"Altos" });
      setFilas(prev=> prev.map(f=>{
        if (f.riesgo === "Peligro de fuga") return { ...f, valoracion:"Grave", indicadores: { ...f.indicadores, "sin-domicilio":true, "hist-evasion":true, "recursos-fuga":true, "pena-elevada":true }, evidencia:"Registro de rebeldía previa y recursos para abandonar el país", medidasSugeridas:["Arresto domiciliario total","Localizador electrónico"], propuesta:"Detención provisional" };
        if (f.riesgo === "Obstaculización de la investigación") return { ...f, valoracion:"Moderado", indicadores:{ ...f.indicadores, "coordinacion-coimputados":true }, evidencia:"Comunicación sostenida con coimputados", medidasSugeridas:["Prohibición de acercamiento","Presentación periódica"], propuesta:"Subsidiaria a la detención" };
        return f;
      }));
    } else if (id === "defensa-riesgo-bajo") {
      setRol("Defensa");
      setMeta(m=>({...m, delito:"Estafa", imputado:"María L.", numeroCausa:"2025-900002" }));
      setArraigo({ domicilioFijo:true, tiempoResidenciaMeses:84, empleoFormal:true, contratoIndefinido:true, familiaPrimariaEnPA:true, estudiosActivos:false, antecedentesEvasion:false, pasaporteVigente:false, recursosParaFugarse:"Bajos" });
      setFilas(prev=> prev.map(f=> ({ ...f, valoracion:"Leve", indicadores: Object.fromEntries(Object.keys(f.indicadores).map(k=>[k,false])), evidencia:"", medidasSugeridas:["Presentación periódica","Prohibición de salida del país"], propuesta:"Medidas no privativas" })));
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 space-y-6">
        ...
        {/* Paso 2: Arraigo */}
        <Box>
          <Head title="Paso 2 – Arraigo" description="Completa los factores de arraigo para ponderar el peligro de fuga (CPP art. 221 y concordantes)" />
          <Section className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div>
                <Label>Domicilio fijo</Label>
                <Select value={String(arraigo.domicilioFijo)} onChange={(v:any)=>setArraigo({...arraigo, domicilioFijo: v==="true"})} options={["true","false"]} />
              </div>
              <div>
                <Label>Meses de residencia</Label>
                <Input type="number" value={arraigo.tiempoResidenciaMeses} onChange={(e:any)=>setArraigo({...arraigo, tiempoResidenciaMeses:Number(e.target.value||0)})} />
              </div>
              <div>
                <Label>Empleo formal</Label>
                <Select value={String(arraigo.empleoFormal)} onChange={(v:any)=>setArraigo({...arraigo, empleoFormal: v==="true"})} options={["true","false"]} />
              </div>
              <div>
                <Label>Contrato indefinido</Label>
                <Select value={String(arraigo.contratoIndefinido)} onChange={(v:any)=>setArraigo({...arraigo, contratoIndefinido: v==="true"})} options={["true","false"]} />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div>
                <Label>Familia primaria en Panamá</Label>
                <Select value={String(arraigo.familiaPrimariaEnPA)} onChange={(v:any)=>setArraigo({...arraigo, familiaPrimariaEnPA: v==="true"})} options={["true","false"]} />
              </div>
              <div>
                <Label>Estudios activos</Label>
                <Select value={String(arraigo.estudiosActivos)} onChange={(v:any)=>setArraigo({...arraigo, estudiosActivos: v==="true"})} options={["true","false"]} />
              </div>
              <div>
                <Label>Antecedentes de evasión</Label>
                <Select value={String(arraigo.antecedentesEvasion)} onChange={(v:any)=>setArraigo({...arraigo, antecedentesEvasion: v==="true"})} options={["true","false"]} />
              </div>
              <div>
                <Label>Pasaporte vigente</Label>
                <Select value={String(arraigo.pasaporteVigente)} onChange={(v:any)=>setArraigo({...arraigo, pasaporteVigente: v==="true"})} options={["true","false"]} />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div>
                <Label>Recursos para fugarse</Label>
                <Select value={arraigo.recursosParaFugarse} onChange={(v:any)=>setArraigo({...arraigo, recursosParaFugarse:v})} options={["Bajos","Medios","Altos"]} />
              </div>
              <div className="md:col-span-3 flex items-end">
                <div className="text-sm p-3 bg-slate-800/50 rounded-xl border border-white/10 w-full">
                  <div><b>Puntaje de arraigo:</b> {arraigoScore}/100</div>
                  <div><b>Clasificación:</b> {arraigoNivel}</div>
                </div>
              </div>
            </div>
          </Section>
            ...
            {(filas as FilaRiesgo[]).map((fila, idx)=> (
              <div key={fila.riesgo} className="p-3 rounded-xl border border-white/10 bg-slate-900/40">
                    ...
                    <div className="grid grid-cols-1 gap-1 max-h-40 overflow-auto p-2 bg-slate-800/40 rounded-lg border border-white/10">
                      {MEDIDAS_ALTERNATIVAS.map(m=> (
                        <label key={m} className="flex items-center gap-2 text-xs">
                          <input type="checkbox" checked={fila.medidasSugeridas.includes(m)} onChange={(e:any)=>{
                ...
                <div className="mt-3">
                  ...
                  <Input value={fila.propuesta} onChange={(e:any)=>setPropuesta(idx, e.target.value)} placeholder={rol==="Fiscalía"?"Detención provisional / subsidiarias":"Medidas no privativas concretas…"} />
                </div>
              </div>
            ))}
          </Section>
