...
function puntuarArraigo(a: Arraigo) {
  ...
  if (a.esNacional) score += 15; // nacionalidad suma
  if (a.tieneOtrasCiudadanias) score -= 5; // puede facilitar movilidad
  if (a.residenciaLegalExtranjero) score -= 10; // vínculo externo
  score += a.estatusMigratorioPanama === "Regular" ? 10 : -15; // irregularidad resta

  // Pesos clásicos
  if (a.domicilioFijo) score += 20;
  if (!a.esNacional) {
    // Solo si NO es nacional consideramos antigüedad de residencia local como peso adicional
...
function redactarConclusiones(rol: Rol, filas: FilaRiesgo[], arraigoScore: number) {
  ...
  if (rol === "Fiscalía") {
    ...
    return (
      base +
      ` Se proponen medidas menos gravosas: ${MEDIDAS_ALTERNATIVAS.slice(0, 4).join(", ")}, en atención a un arraigo ${nivelArraigo.toLowerCase()} y la ausencia de riesgos graves.`
    );
  }
  if (rol === "Defensa") {
    const base =
    ...
    if (graves.length === 0 && nivelArraigo !== "Bajo") {
      return (
        ...
        ].join(", ")}, pues el arraigo es ${nivelArraigo.toLowerCase()} y los indicadores no alcanzan umbral de gravedad.`
      );
    }
    return (
      base +
...
Notas: La detención es excepcional y sujeta a control judicial estricto. Se deben preferir medidas menos gravosas cuando neutralicen el riesgo (CPP arts. 221, 227 y concordantes).`;
  ...
  async function descargarPDF() {
    ...
    try {
      ...
      const contentStyled = `Times New Roman
` + informe;
                          ...
                          <input
                            ...
                            onChange={(e: any) => {
                              const next = e.target.checked
                                ? [...fila.medidasSugeridas, m]
                                : fila.medidasSugeridas.filter((x) => x !== m);
                              setMedidas(idx, next);
                            }}
                          />
                          <span>{m}</span>
